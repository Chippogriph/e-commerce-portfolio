...
export const addToCart = (req, res) => {
  ...
  const existingProduct = dbCart
    .prepare("SELECT * FROM cart WHERE slug = ?")
    .get(slug);

  if (existingProduct) {
    dbCart
      .prepare("UPDATE cart SET quantity = quantity + 1 WHERE slug = ?")
      .run(slug);
  } else {
    dbCart
      .prepare(
        "INSERT INTO cart (slug, name, url, brand, price, quantity) VALUES (?, ?, ?, ?, ?, ?)"
      )
      .run(slug, name, url, brand, price, 1);
  }
...
export const updateCart = (req, res) => {
  ...
  const cart = dbCart.prepare("SELECT * FROM cart").all();
  res.json(cart);
};
